...
def compute_distances(Xtrain, X):
	...
	- dists: A numpy array of shape (num_test, num_train) where dists[i, j]
	  ...
	  point.
	"""
	val = np.sqrt(np.sum((np.repeat(X, np.shape(Xtrain)[0], axis=0) - np.tile(Xtrain, (np.shape(X)[0], 1))) ** 2, axis=1))
	dist = np.reshape(val,(np.shape(X)[0], np.shape(Xtrain)[0]))
	return dist
...
def predict_labels(k, ytrain, dists):
	...
	np.argmax(np.bincount( 	ypred = ytrain[np.argmax(np.argsort(np.apply_along_axis(lambda x : np.bincount(x, minlength=dists.shape[1]), 1, dists),axis=1)[:,:k], axis=1)]

	"""
	ypred = np.argmax(np.apply_along_axis(lambda x : np.bincount(x, minlength=dists.shape[1]), 1, ytrain[np.argsort(dists,axis=1)[:,:k]]), axis=1)
	return ypred
