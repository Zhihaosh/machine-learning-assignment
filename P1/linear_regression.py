...
def linear_regression_noreg(X, y):
  ...
  tmp2 = np.matmul(tmp1,np.transpose(X))
  w = np.matmul(tmp2, y)
  return w
...
def regularized_linear_regression(X, y, lambd):
  ...
  #				 YOUR CODE HERE					                    #
  #####################################################		 
  tmp1 = np.linalg.inv(np.matmul(np.transpose(X),X) + lambd * np.identity(12))
  tmp2 = np.matmul(tmp1,np.transpose(X))
  w = np.matmul(tmp2, y)
  return w
...
def tune_lambda(Xtrain, ytrain, Xval, yval, lambds):
  ...
  minErr = float("inf")
  for lambd in lambds:
    tmp1 = np.linalg.inv(np.matmul(np.transpose(Xtrain),Xtrain) + lambd * np.identity(12))
    tmp2 = np.matmul(tmp1,np.transpose(Xtrain))
    w = np.matmul(tmp2, ytrain)
    err = np.sum((yval - np.matmul(Xval, np.transpose(w)))**2) / np.shape(Xval)[0]
    if(err < minErr):
      bestlambda = lambd
      minErr = err
  ####################################################
...
def test_error(w, X, y):
  """
    ...
    """
  err = np.sum((y - np.matmul(X, np.transpose(w)))**2) / np.shape(X)[0]
  return err
